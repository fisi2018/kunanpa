...
export const getFlowersId: FetcherWithoutBody<string[]> = async () => {
    try {
        const { data: firstPage } = await kunanpa.get<ResponseFlowers>(
            '/flores'
        )
        console.log('first page ', firstPage)
        const idsFirstPage = createFlowersIdsAdapter(firstPage)
        const linkOtherPages = firstPage.links.filter(
            link =>
                !link.active &&
                link.url !== null &&
                link.label !== 'Siguiente &raquo;'
        ) as { active: boolean; url: string; label: string }[]
        console.log('links other pages ', linkOtherPages)
        const otherPages = await Promise.all(
            linkOtherPages.map(link =>
                getFlowersByPage(link.url.split('/').pop() as string)
            )
        )
        ...
        console.log('error en get flowers id', e)
